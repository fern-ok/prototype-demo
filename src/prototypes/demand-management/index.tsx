 ...
 * @mode axure
 *
 * 后台需求管理列表，分配给数据基础设施运营方角色查看并处理数据需求。
 */
...
type DemandStatus = '待上报' | '已上报' | '已响应' | '意向达成';
interface DemandRecord {
  ...
  contactPhone: string;
  description: string;
  response?: { responder: string; note: string; data: ResponseDataRow[] };
...
const seed: DemandRecord[] = [
  ...
  { id: 2, name: '城市交通拥堵分析数据需求', industry: '交通运输', budget: '30,000 元', deadline: '2026-09-20', status: '已响应', createTime: '2026-08-16 14:10:00', updateTime: '2026-08-17 10:30:00', requester: '湖南慧行信息技术有限公司', contact: '陈昊', contactPhone: '13973120002', description: '需要城市路网流量、速度和事件数据，支持交通态势分析。', response: { responder: '湖南数据产业集团有限公司', note: '已完成数据资源匹配，可提供道路流量和交通事件数据。', data: [{ id: 1, type: '数据资源', code: 'TRAFFIC-001', name: '城市道路流量数据' }] } },
  { id: 3, name: '文旅客流趋势预测数据需求', industry: '文化旅游', budget: '面议', deadline: '2026-10-15', status: '意向达成', createTime: '2026-08-12 11:05:00', updateTime: '2026-08-15 16:42:00', requester: '潇湘文旅发展集团', contact: '周琳', contactPhone: '13773120003', description: '面向景区客流预测和营销分析，关注节假日客流及来源地数据。', response: { responder: '湖南慧行信息技术有限公司', note: '可提供文旅客流及来源地分析数据。', data: [{ id: 1, type: '数据产品', code: 'TOUR-003', name: '景区客流分析产品' }] }, intent: { scale: '2.5', scaleUnit: 'GB', note: '双方就数据使用范围和交付方式达成初步意向。' } },
  ...
  { id: 5, name: '农业产销监测数据需求', industry: '农业农村', budget: '20,000 元', deadline: '2026-09-08', status: '已响应', createTime: '2026-08-06 08:45:00', updateTime: '2026-08-08 13:20:00', requester: '湖南农服数字科技有限公司', contact: '李娟', contactPhone: '13573120005', description: '关注主要农产品产量、价格与物流流向，服务农产品产销预测。', response: { responder: '湖南农服数字科技有限公司', note: '已匹配农产品产量、价格和物流数据资源。', data: [{ id: 1, type: '数据资源', code: 'AGRI-015', name: '农产品产销监测数据' }] } },
  { id: 6, name: '生态环境质量评价数据需求', industry: '生态环境', budget: '面议', deadline: '2026-08-30', status: '意向达成', createTime: '2026-08-02 10:15:00', updateTime: '2026-08-05 17:10:00', requester: '湖南绿洲环保研究院', contact: '赵峰', contactPhone: '13373120006', description: '需要空气、水质、土壤等环境监测数据，用于区域生态评价。', response: { responder: '湖南数据产业集团有限公司', note: '已提供环境监测数据服务方案。', data: [{ id: 1, type: '数据产品', code: 'ECO-020', name: '生态环境质量评价数据产品' }] }, intent: { scale: '800', scaleUnit: 'MB', note: '已确认合作意向，后续将推进协议签订。' } }
  ,{ id: 7, name: '园区能耗监测数据需求', industry: '节能环保', budget: '10,000-20,000 元', deadline: '2026-10-31', status: '待上报', createTime: '2026-08-20 10:00:00', updateTime: '2026-08-20 10:00:00', requester: '湖南绿色园区运营有限公司', contact: '李明', contactPhone: '13273120007', description: '需要园区企业用电、用水及碳排放监测数据，支持节能管理。' }
];

const statusClass = (status: DemandStatus) => status === '已上报' || status === '待上报' ? 'status-pending' : status === '已响应' ? 'status-reviewing' : 'status-approved';
...
const OriginalComponent = () => {
  ...
  const [responseResponder, setResponseResponder] = useState('');
  const [responseDataType, setResponseDataType] = useState<'' | '数据资源' | '数据产品'>('');
  ...
  const addResponseRow = () => setResponseRows(prev => [...prev, { id: Date.now(), type: responseDataType, code: '', name: '' }]);
  const deleteResponseRow = (id: number) => setResponseRows(prev => prev.length > 1 ? prev.filter(row => row.id !== id) : prev);
  const submitResponse = () => {
    const errors: Record<string, string> = {};
    if (!responseResponder) errors.responder = '请选择应答方';
    ...
    setRecords(prev => prev.map(item => item.id === responseRecord.id ? { ...item, status: '已响应', updateTime: now, response: { responder: responseResponder, note: responseNote, data: responseRows } } : item));
    setResponseRecord(null);
  };
  const report = (record: DemandRecord) => setRecords(prev => prev.map(item => item.id === record.id ? { ...item, status: '已上报', updateTime: new Date().toISOString().slice(0, 19).replace('T', ' ') } : item));
    ...
    <div className="modal-large form-modal response-modal" onClick={e => e.stopPropagation()}>
      ...
      <div className="modal-body form-modal-body">
        <div className="form-section"><div className="form-grid">
          <div className="form-item"><label>需求标题</label><input type="text" value={responseRecord.name} disabled /></div>
          <div className="form-item"><label>应答方<span className="required">*</span></label><div><select value={responseResponder} onChange={e => { const responder = e.target.value; const type = responder === '湖南慧行信息技术有限公司' ? '数据产品' : '数据资源'; setResponseResponder(responder); setResponseDataType(responder ? type : ''); setResponseRows(prev => prev.map(row => ({ ...row, type: responder ? type : '' }))); setResponseErrors(prev => ({ ...prev, responder: '' })); }} className={responseErrors.responder ? 'has-error' : ''}><option value="">请选择应答方</option><option>湖南数据产业集团有限公司</option><option>湖南慧行信息技术有限公司</option><option>湖南农服数字科技有限公司</option></select>{responseErrors.responder && <span className="error-text">{responseErrors.responder}</span>}</div></div>
        ...
        <div className="form-section"><h4 className="section-title"><span className="title-bar"></span>响应数据<button className="btn-add-row" onClick={addResponseRow}>+ 新增</button></h4><div className="product-table-wrapper"><table className="product-form-table response-data-table"><thead><tr><th className="col-seq">序号</th><th>数据类型</th><th><span className="required">*</span> 数据名称</th><th>数据标识</th><th className="col-op">操作</th></tr></thead>
          <tbody>{responseRows.map((row, index) => <tr key={row.id}>
            <td className="col-seq">{index + 1}</td>
            <td><select value={row.type} disabled className={responseErrors[`row_${row.id}_type`] ? 'has-error' : ''}>
              <option value="">请选择应答方后自动填充</option><option value="数据资源">数据资源</option><option value="数据产品">数据产品</option></select>{responseErrors[`row_${row.id}_type`] && <span className="error-text">{responseErrors[`row_${row.id}_type`]}</span>}</td><td>
              <input type="text" value={row.name} placeholder="请输入数据名称" onChange={e => updateResponseRow(row.id, 'name', e.target.value)} className={responseErrors[`row_${row.id}_name`] ? 'has-error' : ''} />{responseErrors[`row_${row.id}_name`] && <span className="error-text">{responseErrors[`row_${row.id}_name`]}</span>}</td><td>
              <input type="text" value={row.code} placeholder="填写数据名称后自动生成" disabled /></td><td className="col-op"><button className="btn-delete-row" onClick={() => deleteResponseRow(row.id)}>×</button></td></tr>)}</tbody></table></div></div>
      </div>
  ...
  const renderDemandViewModal = () => view && <div className="modal-overlay" onClick={() => setView(null)}><div className="modal-large view-modal demand-view-modal" onClick={e => e.stopPropagation()}><div className="modal-header"><h3>查看需求</h3><button className="modal-close" onClick={() => setView(null)}>×</button></div><div className="modal-body demand-view-body">{(view.response || view.intent) && <div className="demand-view-tabs"><button className={viewTab === 'basic' ? 'active' : ''} onClick={() => setViewTab('basic')}>基本信息</button>{view.response && <button className={viewTab === 'response' ? 'active' : ''} onClick={() => setViewTab('response')}>响应信息</button>}{view.intent && <button className={viewTab === 'intent' ? 'active' : ''} onClick={() => setViewTab('intent')}>意向信息</button>}</div>}{viewTab === 'basic' && <div className="demand-info-table"><div className="info-cell"><span>需求标题</span><b>{view.name}</b></div><div className="info-cell"><span>行业分类</span><b>{view.industry}</b></div><div className="info-cell"><span>资源预算</span><b>{view.budget}</b></div><div className="info-cell"><span>需求截止日期</span><b>{view.deadline}</b></div><div className="info-cell"><span>需求状态</span><b>{view.status}</b></div><div className="info-cell"><span>创建时间</span><b>{view.createTime}</b></div><div className="info-cell"><span>更新时间</span><b>{view.updateTime}</b></div><div className="info-cell"><span>联系人</span><b>{view.contact}</b></div><div className="info-cell"><span>联系方式</span><b>{view.contactPhone}</b></div><div className="info-cell full"><span>需求描述</span><b>{view.description}</b></div></div>}{viewTab === 'response' && view.response && <><div className="demand-info-table"><div className="info-cell"><span>应答方</span><b>{view.response.responder}</b></div><div className="info-cell full"><span>响应说明</span><b>{view.response.note}</b></div></div><table className="demand-view-table"><thead><tr><th>序号</th><th>数据类型</th><th>数据名称</th><th>数据标识</th></tr></thead>
    <tbody>{view.response.data.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td>{item.type}</td>
      <td>{item.name}</td><td>{item.code}</td></tr>)}</tbody></table></>}{viewTab === 'intent' && view.intent && <div className="demand-info-table"><div className="info-cell"><span>数据规模</span><b>{view.intent.scale} {view.intent.scaleUnit}</b></div><div className="info-cell full"><span>意向说明</span><b>{view.intent.note}</b></div></div>}</div><div className="modal-footer"><button className="btn btn-default" onClick={() => setView(null)}>关闭</button></div></div></div>;

  return <Layout activeMenu="demand-management" breadcrumb="需求管理" title="需求管理" role={role} onRoleChange={setRole} roleOptions={['数据基础设施运营方']} specContent={specContent} changeLogContent={changeLogContent}>
    {renderDemandViewModal()}
    <div className="filter-section">
      <div className="filter-row"><div className="filter-item"><label>需求标题</label><input type="text" placeholder="请输入需求标题" value={name} onChange={e => { setName(e.target.value); setPage(1); }} /></div><div className="filter-item"><label>资源预算</label><input type="text" placeholder="请输入预算" value={budget} onChange={e => { setBudget(e.target.value); setPage(1); }} /></div><div className="filter-item filter-item-select"><label>需求状态</label><select value={status} onChange={e => { setStatus(e.target.value); setPage(1); }}><option value="">请选择</option><option>待上报</option><option>已上报</option><option>已响应</option><option>意向达成</option></select></div>{dateRange('需求截止日期', deadlineStart, deadlineEnd, setDeadlineStart, setDeadlineEnd)}</div>
