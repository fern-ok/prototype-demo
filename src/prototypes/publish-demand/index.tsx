...
type Demand = { id: number; title: string; industry: string; budget: string; budgetValue?: number; deadline: string; description: string; dataTypes: string[]; expectedResponder?: string };
...
const responderOptions = ['湖南数据产业集团有限公司', '湖南慧行信息技术有限公司', '湖南农服数字科技有限公司', '湖南绿洲环保研究院'];
const createForm = () => ({ title: '', dataType: '', industry: '', budgetMode: 'price', budget: '', description: '', deadline: '', contact: '', phone: '', isPublic: 'yes', expectedResponder: '' });
...
const OriginalComponent = () => {
  ...
  const submit = () => {
    ...
    if (!form.dataType) nextErrors.dataType = '请选择需求数据分类';
    ...
    if (form.isPublic === 'no' && !form.expectedResponder) nextErrors.expectedResponder = '请选择期望应答方';
    if (Object.keys(nextErrors).length) return setErrors(nextErrors);
    setDemands(previous => [{ id: Date.now(), title: form.title.trim(), industry: form.industry, budget: form.budgetMode === 'negotiable' ? '面议' : `${Number(form.budget).toLocaleString('zh-CN')} 元`, budgetValue: form.budgetMode === 'negotiable' ? undefined : Number(form.budget), deadline: form.deadline, description: form.description.trim(), dataTypes: [form.dataType], expectedResponder: form.isPublic === 'no' ? form.expectedResponder : undefined }, ...previous]);
    setIndustry('全部'); setBudgets([]); setPage(1); closeModal();
  };

  return <PortalLayout className="publish-demand-page" activeNav="demand" specContent={specContent} changeLogContent={changeLogContent}>
    ...
    {isModalOpen && <div className="publish-modal-overlay" onClick={closeModal}><section className="publish-modal" onClick={event => event.stopPropagation()}><header><h2>发布需求</h2><button onClick={closeModal} aria-label="关闭"><X size={20} /></button></header>
    <div className="publish-form">
      <FormItem label="需求标题" required error={errors.title}>
        <input value={form.title} placeholder="请输入标题" onChange={event => setField('title', event.target.value)} />
      </FormItem>
      <FormItem label="需求数据分类" required error={errors.dataType}>
        <div className="form-choice-row">{['数据资源', '数据产品'].map(type => <label key={type}><input type="radio" checked={form.dataType === type} onChange={() => setField('dataType', type)} />{type}</label>)}</div>
      </FormItem>
      <FormItem label="行业分类" required error={errors.industry}>
        <select value={form.industry} onChange={event => setField('industry', event.target.value)}>
          <option value="">请选择</option>{industries.slice(1).map(item => <option key={item}>{item}</option>)}</select>
      </FormItem>
      <FormItem label="资金预算" required error={errors.budget}><div className="budget-control"><label><input type="radio" checked={form.budgetMode === 'price'} onChange={() => setField('budgetMode', 'price')} />输入价格</label><label><input type="radio" checked={form.budgetMode === 'negotiable'} onChange={() => setField('budgetMode', 'negotiable')} />面议</label>{form.budgetMode === 'price' && <div><input type="number" min="0" value={form.budget} placeholder="请输入金额" onChange={event => setField('budget', event.target.value)} /><span>元</span></div>}</div></FormItem>
      <FormItem label="需求描述" required error={errors.description}><textarea rows={4} value={form.description} placeholder="请输入需求描述" onChange={event => setField('description', event.target.value)} /></FormItem>
      <FormItem label="需求截止日期" required error={errors.deadline}><input type="date" min="2026-08-20" value={form.deadline} onChange={event => setField('deadline', event.target.value)} /></FormItem>
      <div className="form-two-columns">
        <FormItem label="联系人" required error={errors.contact}>
          <input value={form.contact} placeholder="请输入联系人" onChange={event => setField('contact', event.target.value)} />
        </FormItem>
        <FormItem label="联系方式" required error={errors.phone}>
          <input value={form.phone} placeholder="请输入联系方式" onChange={event => setField('phone', event.target.value)} />
        </FormItem>
      </div>
      <div className="">
        {/*
        <FormItem label="是否公开" required>
          <div className="form-choice-row">
            <label>
              <input type="radio" checked={form.isPublic === 'yes'} onChange={() => setField('isPublic', 'yes')} />是</label>
            <label>
              <input type="radio" checked={form.isPublic === 'no'} onChange={() => setField('isPublic', 'no')} />否</label>
          </div>
        </FormItem>
        */}
        <FormItem label="期望应答方" required error={errors.expectedResponder}>
          <select value={form.expectedResponder} onChange={event => setField('expectedResponder', event.target.value)}>
            <option value="">请选择期望应答方</option>{responderOptions.map(item => <option key={item}>{item}</option>)}</select>
        </FormItem>
      </div></div><footer><button className="cancel" onClick={closeModal}>取消</button><button className="submit" onClick={submit}>提交</button></footer></section></div>}
  </PortalLayout>;
};
