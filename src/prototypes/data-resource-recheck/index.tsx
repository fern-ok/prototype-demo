 ...
 * @mode axure
 *
 * 数据资源复审列表，支持待处理和已处理两个TAB页，分配给实施机构角色
 */
...
const seedData: ResourceRecord[] = [
  // 待复审
  {
    ...
    reviewStatus: '待复审',
    initialResult: '初审通过',
    initialOpinion: '材料齐全，资源目录清晰，同意进入复审。',
    initialReviewer: '李初审员',
    initialReviewTime: '2026-07-29 11:30:45'
  },
  {
    id: 2,
    authName: '医保结算数据授权使用申请',
    initiator: '实施机构',
    resourceCatalog: '医保结算数据;门诊诊疗记录',
    createTime: '2026-07-22 09:00:00',
    updateTime: '2026-07-28 14:20:00',
    reviewStatus: '待复审',
    initialResult: '初审通过',
    initialOpinion: '初审通过，数据使用范围合理。',
    initialReviewer: '张初审员',
    initialReviewTime: '2026-07-28 14:20:00'
  },
  {
    id: 3,
    authName: '交通流量数据资源授权申请',
    initiator: '运营机构',
    resourceCatalog: '城市交通流量数据;公共交通刷卡记录',
    createTime: '2026-07-20 13:30:00',
    updateTime: '2026-07-27 10:15:20',
    reviewStatus: '待复审',
    initialResult: '初审通过',
    initialOpinion: '初审通过。',
    initialReviewer: '李初审员',
    initialReviewTime: '2026-07-27 10:15:20'
  },
  {
    id: 4,
    authName: '教育评估数据授权使用申请',
    initiator: '实施机构',
    resourceCatalog: '学生成绩数据;教师评估数据',
    createTime: '2026-07-18 11:00:00',
    updateTime: '2026-07-26 15:40:30',
    reviewStatus: '待复审',
    initialResult: '初审通过',
    initialOpinion: '初审通过，材料完整。',
    initialReviewer: '张初审员',
    initialReviewTime: '2026-07-26 15:40:30'
  },
  // 待初审
  {
    id: 5,
    ...
    initiator: '运营机构',
    resourceCatalog: '空气质量监测数据;水质检测数据',
  ...
  // 初审不通过
  {
    ...
    reviewStatus: '初审不通过',
    initialResult: '初审不通过',
    initialOpinion: '资源目录范围不明确，需补充数据使用场景说明。',
    initialReviewer: '张初审员',
    initialReviewTime: '2026-07-30 14:20:15'
  },
  // 复审不通过
  {
    id: 7,
    ...
    initialOpinion: '初审通过，数据使用范围合理。',
    initialReviewer: '张初审员',
    initialReviewTime: '2026-07-24 15:10:20',
    recheckResult: '复审不通过',
    recheckOpinion: '数据安全评估不达标，需补充数据安全防护方案。',
    recheckReviewer: '王复审员',
    recheckTime: '2026-07-28 09:45:30'
  },
  // 复审通过
  {
    id: 8,
    ...
    initialOpinion: '初审通过。',
    initialReviewer: '张初审员',
    initialReviewTime: '2026-07-18 11:20:30',
    recheckResult: '复审通过',
    recheckOpinion: '复审通过。',
  ...
  },
  {
    ...
    reviewStatus: '复审通过',
    initialResult: '初审通过',
    initialOpinion: '初审通过，资源授权范围明确。',
    initialReviewer: '李初审员',
    initialReviewTime: '2026-07-15 09:40:50',
    recheckResult: '复审通过',
    recheckOpinion: '复审通过，符合数据资源授权运营要求。',
    recheckReviewer: '王复审员',
    recheckTime: '2026-07-19 10:30:15'
  }
];
...
const OriginalComponent = () => {
  ...
  const [activeMenu] = useState<'data-resource-recheck'>('data-resource-recheck');
  ...
  const [recheckForm, setRecheckForm] = useState({ recheckResult: '', recheckOpinion: '' });
  ...
  const handleRecheck = (record: ResourceRecord) => {
    setCurrentRecord(record);
    setRecheckForm({ recheckResult: '', recheckOpinion: '' });
    setRecheckErrors({});
  ...
  const handleRecheckSubmit = () => {
    ...
    if (!recheckForm.recheckResult) errors.recheckResult = '请选择复审结果';
    if (Object.keys(errors).length > 0) {
      setRecheckErrors(errors);
      return;
    }

    const finalStatus = recheckForm.recheckResult === '复审通过' ? '复审通过' : '复审不通过';
    const now = new Date();
    const pad = (n: number) => String(n).padStart(2, '0');
    const recheckTime = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;

    setRecords(prev => prev.map(r => r.id === currentRecord!.id ? {
      ...r,
      reviewStatus: finalStatus,
      recheckResult: recheckForm.recheckResult,
      recheckOpinion: recheckForm.recheckOpinion,
      recheckReviewer: role,
      recheckTime,
      updateTime: recheckTime
    } : r));
                ...
                <tr key={record.id}>
                  ...
                  <td className="col-action">
                    <div className="action-buttons">
                      ...
                      {activeTab === 'pending' && (
                        <button className="action-btn" onClick={() => handleRecheck(record)}>复审</button>
                      )}
                    </div>
                  </td>
                </tr>
    ...
    <div className="modal-overlay" onClick={() => setShowRecheckModal(false)}>
      <div className="modal-medium" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h3>复审办理</h3>
          <button className="modal-close" onClick={() => setShowRecheckModal(false)}>×</button>
        </div>
        <div className="modal-body">
          ...
          <div className="form-item">
            <label>复审结果 <span className="required">*</span></label>
            <select
              value={recheckForm.recheckResult}
              onChange={(e) => {
                setRecheckForm(prev => ({ ...prev, recheckResult: e.target.value }));
                if (recheckErrors.recheckResult) setRecheckErrors({});
              }}
              className={recheckErrors.recheckResult ? 'has-error' : ''}
            >
              <option value="">请选择</option>
              {RECHECK_RESULTS.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
            {recheckErrors.recheckResult && <span className="error-text">{recheckErrors.recheckResult}</span>}
          </div>
          <div className="form-item">
            <label>复审意见</label>
            <textarea
              rows={4}
              placeholder="请输入复审意见"
              value={recheckForm.recheckOpinion}
              onChange={(e) => setRecheckForm(prev => ({ ...prev, recheckOpinion: e.target.value }))}
            />
          </div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-default" onClick={() => setShowRecheckModal(false)}>取消</button>
          <button className="btn btn-primary" onClick={handleRecheckSubmit}>确定</button>
        </div>
      </div>
    </div>
  );
